...
impl AmgSource {
    ...
    fn slug_from_hls_url(hls_url: &str) -> Option<String> {
        ...
        let segment = without_fragment.rsplit('/').next().unwrap_or(without_fragment);
        if segment.is_empty() {
            return None;
        }
    ...
    pub fn get_known_stations() -> Vec<RadioStation> {
        let known_stations_data = vec![
            ("ruwave", "Русская Волна", "https://ruwave.amgradio.ru/ruwave", "https://volna.top/logoradio/ruwave.svg"),
            ("hypefm", "ХАЙП FM", "https://hfm.amgradio.ru/HypeFM", "https://volna.top/logoradio/hypefm.svg"),
            ("remixfm", "Remix FM", "https://remix.amgradio.ru/Remix", "https://volna.top/logoradio/remixfm.svg"),
            ("escapefm", "Escape FM", "https://escape.amgradio.ru/Escape", "https://volna.top/logoradio/escapefm.svg"),
            ("rusrock", "Русский Рок", "https://rock.amgradio.ru/RusRock", "https://volna.top/logoradio/rusrock.svg"),
            ("jazzfm", "Jazz FM", "https://jazz.amgradio.ru/Jazz", "https://volna.top/logoradio/jazzfm.svg"),
            ("classicfm", "Classic FM", "https://classic.amgradio.ru/Classic", "https://volna.top/logoradio/classicfm.svg"),
            ("popfm", "Pop FM", "https://pop.amgradio.ru/Pop", "https://volna.top/logoradio/popfm.svg"),
        ];
...
impl RadioSourceTrait for AmgSource {
        ...
        if let Some(stations_array) = response.as_array() {
            for station_json in stations_array {
                if let (Some(title_obj), Some(meta_obj)) = (
                    ...
                    let name = title_obj.as_str().unwrap_or("Неизвестная станция").to_string();
                    let slug = station_json.get("slug").and_then(|s| s.as_str()).map(String::from);
                    
                    let stream_hls = meta_obj.get("stream_hls").and_then(|s| s.as_str()).map(String::from);
                    ...
                    let station_slug = meta_key.clone().unwrap_or_else(|| slug.clone().unwrap_or_default());
                    
                    // Получаем логотип через media API
                    let logo = if let Some(media_id) = station_json.get("featured_media").and_then(|m| m.as_u64()) {
                        let media_url = format!("https://ru.volna.top/wp-json/wp/v2/media/{}", media_id);
                        if let Ok(media_resp) = self.client.get(&media_url).send().await {
                            if let Ok(media_json) = media_resp.json::<serde_json::Value>().await {
                                media_json.get("source_url").and_then(|s| s.as_str()).map(String::from)
                            } else {
                                None
                            }
                        } else {
    ...
    async fn update_metadata(&self, station: &mut RadioStation) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let slug = station.meta_key.as_ref()
